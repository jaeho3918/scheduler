...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.buf_timer_table = query_off.get_table()
    ...
    def get_table(self):
        ...
        normalTable, completeTable = query_off.get_table()
    ...
    def summitBtn_clicked(self):
        ...
        if not (0 <= self.__STATUS <= len(self.__statusList)) \
            ...
            if self.__statusList_select_switch == False:
                query_off.increase_id()
                self.__MEMOID = query_off.get_id()
                query_off.insert([self.__MEMOID, self.__STATUS, statusContents, rowItem[2], rowItem[3], 0, 0,
                                  self.__datetime.toString("MM월 dd일 dddd  ap hh:mm")])
            else:
                # query_off.update(1,1)
                self.__statusList_select_switch = False
    ...
    def updatetime_alram(self):
        ...
        if self.__timer_count == 5:
            self.__timer_count = 0
            if self.buf_timer_table != query_off.get_table():
                self.buf_timer_table == query_off.get_table()
                self.get_table()
